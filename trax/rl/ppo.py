...
B, scalar   - batch size
T, scalar   - number of time-steps in a trajectory
...
Policy            :: ([B, T + 1] + OBS, [B, T, C]) ->  [B, T, C, A]
Value             :: ([B, T + 1] + OBS, [B, T, C]) ->  [B, T]
Policy and Value  :: ([B, T + 1] + OBS, [B, T, C]) -> ([B, T, C, A], [B, T])
...
def collect_trajectories(env,
  ...
  Args:
    ...
    policy_fn: Callable
      (observations(B,T+1), actions(B, T, C)) -> log-probabs(B, T, C, A).
...
def run_policy(
  ...
  # TODO(pkozakowski): Pass the actual actions here, to enable autoregressive
  # action sampling.
  (batch_size, length) = observations.shape[:2]
  dummy_actions = onp.zeros(
      (batch_size, length - 1) + action_space.shape, dtype=action_space.dtype
  )
  ...
  log_probs = log_probs[np.arange(batch_size), index]
  value_preds = value_preds[np.arange(batch_size), index]
